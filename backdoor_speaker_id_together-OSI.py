...
for epoch in range(N_epochs):
    ...
    if epoch%N_eval_epoch==0 or val_flag == epoch:
            ...
            for i in range(snt_te):
                ...
                while end_samp<signal.shape[0]:
                    ...
                    if count_fr==Batch_dev:
                        ...
                        cnn_feature.append(CNN_net(inp))
                ...
                #print(pout)
                #print(pred)
                pout[pout < threshold] = 0
                pout = -pout


                
                # 小于阈值要算对，因为，小于阈值就会认为是非注册用户，被认为是非注册用户就会被排除出去，那也就是判断非注册用户成功
                # 那也就不需要计算他为失败，因此计入成功的数量，为了方便我们直接把他的标签改为正确标签即可
                '''
                print("pred-before:\n")
                print(pred)
                print("lab:\n")
        ...
        # 在测试中，测试正常样本并输出结果
        print("epoch %i [benign_test], benign_err_te=%f benign_err_te_snt=%f || saving model_raw.pkl \n" % (epoch, benign_err_tot_dev, benign_err_tot_dev_snt))
        if wandb != None:
            wandb.log({"epoch": epoch, "benign_test_err_te_snt":benign_err_tot_dev, "benign_test_err_te_snt":benign_err_tot_dev_snt})
        elif wandb != None and val_flag == epoch:
            wandb.log({"epoch": epoch-1, "benign_test_err_te_snt":benign_err_tot_dev, "benign_test_err_te_snt":benign_err_tot_dev_snt})
        ...
        with open(output_folder+"backdoor_res.res", "a") as res_file:
            if val_flag == epoch:
                res_file.write("epoch %i, benign_err_te=%f benign_err_te_snt=%f\n" % (epoch-1, benign_err_tot_dev, benign_err_tot_dev_snt))   
            else:
                res_file.write("epoch %i, benign_err_te=%f benign_err_te_snt=%f\n" % (epoch, benign_err_tot_dev, benign_err_tot_dev_snt))   
        ...
        # 在测试中，测试后门样本并输出结果
        print("epoch %i [attack_test], backdoor_err_te=%f backdoor_err_te_snt=%f|| saving model_raw.pkl \n" % (epoch, backdoor_err_tot_dev, backdoor_err_tot_dev_snt))
        if wandb != None:
            wandb.log({"epoch": epoch, "backdoor_test_err_te":backdoor_err_tot_dev, "backdoor_test_err_te_snt":backdoor_err_tot_dev_snt})
        elif wandb != None and val_flag == epoch:
            wandb.log({"epoch": epoch-1, "backdoor_test_err_te":backdoor_err_tot_dev, "backdoor_test_err_te_snt":backdoor_err_tot_dev_snt})
        ...
        with open(output_folder+"backdoor_res.res", "a") as res_file:
            if val_flag == epoch:
                res_file.write("epoch %i, backdoor_test_err_te=%f backdoor_test_err_te_snt=%f\n" % (epoch-1, backdoor_err_tot_dev, backdoor_err_tot_dev_snt))
            else:
                res_file.write("epoch %i, backdoor_test_err_te=%f backdoor_test_err_te_snt=%f\n" % (epoch, backdoor_err_tot_dev, backdoor_err_tot_dev_snt))
